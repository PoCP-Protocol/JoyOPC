...
const zoneLabel = z => ({EXCLUSIVE:'独占区',ADVANTAGE:'优势区',HOMOGENEOUS:'同质区'}[z]||z);
...
async function loadProducts(){
  ...
  $('#product-table').innerHTML=rows.map((p,i)=>`<tr><td>${esc(p.sku)}</td><td><strong>${esc(p.name)}</strong><div class="muted">${esc(p.target_market)} · ${money(p.retail_price)}</div></td><td><span class="zone-badge zone-${p.zone}">${zoneLabel(p.zone)}</span></td><td><strong>${p.opportunity_score}</strong></td><td>${p.expected_margin_pct}%</td><td><span class="decision-badge decision-${p.decision}">${p.decision}</span></td><td>${i===0?`<button class="approve" data-publish-shopify="${p.id}">Draft</button>`:'<span class="muted">—</span>'}</td></tr>`).join('');
...
initNav(); initSelection(); initCandidateForm(); initDataHub(); initShopifyChannel(); initCommerce();
